...
try {
    ...
    & $dotNetExecutable publish "src\TaskManager.App\TaskManager.App.csproj" --framework net10.0-windows --configuration Release --runtime win-x64 --self-contained true --no-restore --output $publishDirectory
    if ($LASTEXITCODE -ne 0) { throw "Application publish failed." }
    & $dotNetExecutable publish "src\TaskManager.Cli\TaskManager.Cli.csproj" --framework net10.0 --configuration Release --runtime win-x64 --self-contained true --no-restore --output $publishDirectory
