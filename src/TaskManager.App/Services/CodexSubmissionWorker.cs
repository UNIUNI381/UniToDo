...
/// <summary>Codex送信を1件ずつ実行して結果を通知する。</summary>
    ...
    ILogger<CodexSubmissionWorker> logger) : BackgroundService
{
    ...
    /// <summary>1件の共有会話送信を実行し、失敗時は編集済み本文を確認待ちへ戻す。</summary>
        ...
        try
        {
            ...
            if (result.IsSuccess)
            {
                ...
                string responseMessage = string.IsNullOrWhiteSpace(result.OutputMessage)
                    ? "Codexは正常終了しましたが、応答本文はありませんでした。"
                    : result.OutputMessage;
                notifications.ShowCodexResult(responseMessage, false);
                return;
            }
            applicationLogger.LogWarning("Codex submission failed; retry allowed: {CanRetry}.", result.CanRetry);
            if (result.CanRetry) reviews.RequeueFailedSubmission(submission);
            ...
            string displayedFailure = string.IsNullOrWhiteSpace(failureDetail)
                ? $"Codexが終了コード{result.ExitCode}を返しました。"
                : failureDetail;
            notifications.ShowCodexResult(
                result.CanRetry ? $"{displayedFailure}{Environment.NewLine}{Environment.NewLine}本文を確認画面へ戻しました。" : displayedFailure,
                true);
        }
...
public interface IUserNotificationService
{
    ...
    /// <summary>Codexの応答待ちを中央下の状態画面へ表示する。</summary>
    void ShowCodexProcessing();

    /// <summary>Codexの応答またはエラーを中央下の詳細画面へ表示する。</summary>
