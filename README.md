...
追加バックアップは正本DBだけを保存先の`hourly`（1時間ごと・最大6世代）と`daily`（1日ごと・最大30世代）に保存します。過去のバックアップは含めません。アプリ起動中に動作し、停止中の時間帯は補完しません。失敗時は設定画面に理由を表示し、次の1分周期に再試行します。手動保存も時間別6世代に含みます。OFFや保存先変更で既存ファイルは削除しません。ドライブ文字が違っても同じ物理ディスクなら、そのディスクの故障対策にはなりません。
