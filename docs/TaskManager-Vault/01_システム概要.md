...
flowchart LR
  ...
  Android[Androidブラウザ] <-->|本人限定HTTPS| Serve[Tailscale Serve]
  Serve <-->|ループバック| API
  API -->|SSE変更通知| Browser
  ...
  Automation --> Services
  Automation --> Tray
  Submission --> Chat[CodexChatService]
  Chat <--> CodexServer[CodexAppServer]
  Submission --> Tray
...
| [音声入力](07_音声入力.md) | F13受付、TypeWhisper APIによる録音制御、Ollama並行準備、確認画面、Web共有会話への送信を担うサブシステム |
...
9. F13から始まる音声入力は、TypeWhisperで校正し、利用者確認後にWebと共通のCodexChatServiceへ直列送信する。詳細は[音声入力](07_音声入力.md)を参照する。
10. PC・Androidの「Codexでタスク管理」は、非公開のApp Serverに接続したWeb会話を利用する。送信・確認・停止をUIに依存しないサービスで管理する。詳細は[Codex会話](12_Codex会話.md)を参照する。
...
- 待受はループバック限定。`LocalRequestMiddleware`はローカル要求と、任意設定のTailscale Serve経由の本人要求を区別して検証する。詳細は[スマートフォンアクセス](11_スマートフォンアクセス.md)。
