...
| タブ | 主な機能 |
|---|---|
| ダッシュボード | 未確認の異常終了をCodex共有まで案内する永続警告、仕事・私用・プロジェクト・プロジェクト未設定による絞り込み別の推薦1件、待機中の一時的なタスク強制表示、非実行中の完了確認を含む固定幅の状態操作と右端のタスク編集、Codexタスク管理画面の起動、Google予定・締切・作業ログ、終了済みログの直接編集と実行中ログの開始時刻修正 |
...
| 設定 | 活動時間、重み、Calendar、通知、埋め込みURL、Codex音声入力の共有会話案内、追加バックアップのON/OFF・保存先・結果 |
...
| 追加保全 | `GET /backup/status`、`POST /backup/external` | PC専用の追加バックアップ状態・即時実行 |

追加バックアップの状態は最終確認時刻、区分ごとの最終保存・確認成功時刻、`saved`／`unchanged`の結果、エラーを返します。設定画面では変更なしのスキップと実保存を区別します。
