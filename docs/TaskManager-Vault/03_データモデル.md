...
| 追加バックアップ | 利用者指定フォルダの`hourly`（6世代）・`daily`（30世代）。初期設定OFF |
...
追加バックアップの設定は既存`settings`表の`externalBackupEnabled`（既定false）・`externalBackupDirectory`（既定空文字）に保持します。正本DBの保存先は変更しません。利用者固有の保存先をソースや頒布物へ埋め込みません。世代管理と実行条件は[推薦と自動処理](04_推薦と自動処理.md)を参照してください。
...
タスク・下書きバッチ・プロジェクトの単一取得は対象とその関連情報だけを取得します。ID検索索引は接続時に登録する`TASK_IDENTIFIER`照合（.NETの`OrdinalIgnoreCase`）を使い、大文字小文字とUnicodeの従来互換を維持します。外部SQLiteツールでこれらの索引を検査・利用する場合も同じ照合の登録が必要です。

推薦は未完了タスクと、その依存条件に必要な完了済みタスクのID・状態だけを読み込みます。未完了タスクと計算値が残るタスクには部分索引を使います。計算結果は値が変わった候補だけを更新し、候補から外れた行の計算値を消去します。登録・編集の依存グラフ全体検証は全タスクを使います。版7の索引追加前にはSQLiteバックアップを作成します。
