...
操作規則は [SKILL.md](../SKILL.md)。ここでは必要なコマンド・入力形式だけを確認する。
...
「来週」は選んだ基準の7日後とする。プロジェクト既定を基準にした場合は未指定の曜日・期限種別を維持する。日時を一意に決められなければ登録前に確認する。

日付を指定・解決した期限で時刻未指定ならローカル20:00を補い、オフセット付き日時で渡す。明示された0時などは維持する。期限全体の未指定は `auto` で扱い、日付を勝手に追加しない。
...
```json
{
  ...
  "aliases": [{ "aliasText": "サンプルPJ" }],
  "contextDocuments": [{
    "title": "進行ルール",
    "contentMarkdown": "成果物は提出前に内部レビューを行う。",
    "priority": 5,
    "enabled": true
  }],
  "deadlineRule": {
...
曜日は1（月曜）～7（日曜）。背景情報単独の入力は `contextDocuments` 内のオブジェクト形式を使う。
